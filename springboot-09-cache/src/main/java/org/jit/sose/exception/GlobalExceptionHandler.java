...
public class GlobalExceptionHandler {
	...
	public String DataFormatException(DataFormatException e) {
		...
		e.printStackTrace();
		return ResponseUtil.error(ResponseConstant.DATA_FORMAT_ERROR, e.getMsg());
	}
...
//	@ResponseStatus(HttpStatus.BAD_REQUEST)// 处理400异常类
	...
	public String HttpMessageNotReadableException(HttpMessageNotReadableException e) {
		e.printStackTrace();
		return ResponseUtil.error(ResponseConstant.DATA_FORMAT_ERROR, "参数传入异常");
	}
...
//		LogUtil.logResult("RuntimeException", e);
		e.printStackTrace();
		return ResponseUtil.error(ResponseConstant.ERROR, "全局捕获运行时异常");
	}
...
//		LogUtil.logResult("NullPointerException", e);
		e.printStackTrace();
		return ResponseUtil.error(ResponseConstant.ERROR, "全局捕获空指针异常");
	}
